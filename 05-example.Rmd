...
Below is an example of an explanation showing the same instances as the first example in the Lime method. Two words with the gratest influance are also "dzwi"/"dveri" (door in English) and "bambusowa"/"bambusovy" (bamboo in English), which is reasonable because the name of this cluster is "dveřní zasłona bambus" what can be translated to "bamboo door curtain". These explanations were made using Euclidean metrics and reduced space. This method gave the most valuable explanations, however we later use Bert space and cosine distance for performance reasons.
